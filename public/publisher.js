...
export class Publisher {
  constructor(options = {}) {
    ...
    // Callbacks
    this.onStatusUpdate = options.onStatusUpdate || ((message) => console.log(message));
  ...
  async init() {
    try {
      ...
      this.onStatusUpdate(`Initialization error: ${error.message}`, true);
      throw error;
    }
  }
  ...
  async loadModules() {
    try {
      ...
      // Load Opus audio streamer
      const opusModule = await import(`/stream_audio/opusStreamer.js?t=${Date.now()}`);
      this.initAudioRecorder = opusModule.initAudioRecorder;

      this.onStatusUpdate('All modules loaded successfully');
    } catch (error) {
      this.onStatusUpdate(`Module loading error: ${error.message}`, true);
      throw error;
    }
  }
  ...
  async ensureWasmLoaded() {
    ...
    // Import WASM module
    const { default: init, WasmEncoder } = await import('./pkg/raptorq_wasm');
  ...
  handleOpusAudioChunk(typedArray) {
    ...
    try {
      ...
      if (
        ...
        const timestamp =
          this.opusBaseTime + Math.floor((this.opusSamplesSent * 1000000) / this.kSampleRate);
  ...
  createPacketWithHeader(data, timestamp, type) {
    ...
    const packet = new Uint8Array(
      HEADER_SIZE + (data instanceof ArrayBuffer ? data.byteLength : data.length),
    );
  ...
  trySendMediaConfig() {
    if (!this.mediaConfigSent && this.audioConfig && this.videoConfig) {
      ...
      const vConfigBase64 = this.uint8ArrayToBase64(new Uint8Array(this.videoConfig.description));
